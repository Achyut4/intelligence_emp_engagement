...
class StaffDAO:
    ...
    def searchUserStaff(self, staffVO, roleVO, departmentVO):
        connection = con_db()
        cursor1 = connection.cursor()
        cursor1.execute(
            "SELECT * FROM (((registermaster INNER JOIN departmentmaster ON registermaster.register_DepartmentId = departmentmaster.departmentId) INNER JOIN rolemaster ON registermaster.register_RoleId = rolemaster.roleId)INNER JOIN loginmaster ON registermaster.register_LoginId = loginmaster.loginId) WHERE  register_LoginId='" + staffVO.register_LoginId + "'"
        )
        staffDict = cursor1.fetchall()
        cursor1.close()
        connection.close()
    ...
    def updateUserStaff(self,staffVO):
        connection=con_db()
        cursor1=connection.cursor()
        cursor2 = connection.cursor()
        cursor1.execute(
            "UPDATE registermaster SET registerFirstName='"+staffVO.registerFirstName+"',registerLastName='"+staffVO.registerLastName+"',registerContact='"+staffVO.registerContact+"',registerAddress='"+staffVO.registerAddress+"' WHERE register_LoginId="+staffVO.register_LoginId+""
        )
        cursor2.execute(
            "UPDATE loginmaster SET loginEmail = '"+staffVO.registerEmail+"',loginPassword = '"+staffVO.registerPassword+"'WHERE loginId= '"+staffVO.register_LoginId+"'"
        )
        connection.commit()
        cursor1.close()
        cursor2.close()
